...
app.post("/collection/NFT", async (req, res) => {
  collectionContract = new ethers.Contract(req.body.collectionAddress, CollectionContractJSON.abi, signer)
  res.status(200).json({name: await collectionContract.getCertificatesOfUser(req.body.targetAddress)})
})

app.get("/users-collection", async (req, res) => {
   res.status(200).json({collections: await contract.getUserCollection(req.body.targetAddress)})
})
